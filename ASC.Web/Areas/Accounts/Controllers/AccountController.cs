    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> ServiceEngineers(ServiceEngineerViewModel serviceEngineer)
        {
            ...
            if (serviceEngineer.Registration.IsEdit)
            {
                ...
                if (!result.Succeeded)
                {
                    result.Errors.ToList().ForEach(p =>
                        ModelState.AddModelError("", p.Description));

                    return View(serviceEngineer);
                }

                // Update Password
                if (!string.IsNullOrWhiteSpace(serviceEngineer.Registration.Password))
                {
                    ...
                    {
                        passwordResult.Errors.ToList().ForEach(p =>
                            ModelState.AddModelError("", p.Description));
            ...
            else
            {
                ...
                {
                    roleResult.Errors.ToList().ForEach(p =>
                        ModelState.AddModelError("", p.Description));
            ...
            // Send email after create/update
